...
export default function RackDashboardView({ rackId, selectedDeviceIds }) {
    const normalizedRackId = String(rackId ?? "").trim();
    const isGerminationRack = normalizedRackId.toLowerCase().includes("germination");

    return (
        <>
            <LiveSensorsPanel rackId={normalizedRackId} selectedDeviceIds={selectedDeviceIds} />
            {!isGerminationRack && (
                <As7343TrendsPanel rackId={normalizedRackId} selectedDeviceIds={selectedDeviceIds} />
            )}
            <HistoricalTrendsPanel rackId={normalizedRackId} selectedDeviceIds={selectedDeviceIds} />
        </>
    );
}
